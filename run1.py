...
class PI_DeepONet(nn.Module):
    ...
    def train(self,u1,u2,u_s1, u_s2,dataloader1,dataloader2):
        ...
        self.optimizer = torch.optim.LBFGS(params, lr=0.0000000001,history_size=10, line_search_fn="strong_wolfe",
        ...
        for _ in pbar:
            ...
            for (x_i, t_i,outputs_i),(x_b, t_b, outputs_b) in zip(dataloader1, dataloader2):
                def closure():
                    ...
                    loss =100*pde_loss+1000000*bc_loss
                    loss.backward()
                    return loss
...
Q =400  # number of collocation points for each input sample
M = 5000
r =0.025610
v=0.165856529
T=1
